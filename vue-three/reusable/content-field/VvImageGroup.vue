    ...
    uploaded_files.slice()
        ...
        if(file) {
            ...
            axios.post(store.base_url + '/vaah/manage/media/upload', formData, {
                ...
                if(props.file_limit === 1){
                    content_value.value = res.data.data.full_url;
                }else{
                    content_value.value = [ ...content_value.value,res.data.data.full_url];
                }
...
function removeFile(e){
    ...
    // content_value.value = props.file_limit === 1 ? null : [];
    upload_refs.value.files = [];
    upload_refs.value.uploadedFiles = [];
    upload_refs.value.uploadedFileCount = 0;
...
function removeContent(index){
    ...
    content_value.value.splice(index, 1);
...
<template>
    ...
    <FileUpload name="file"
        ...
        <template #content="{ files, uploadedFiles, removeUploadedFileCallback, removeFileCallback }">
            {{content_value.length}}
            <div v-if="content_value.length > 0"
                 v-for="(file,index) of content_value" :key="index"
                 class="p-fileupload-file gi-upload-file-details">
                <div class="file-warp">
                    <span v-if="file.split('.').pop() === 'png'
                            || file.split('.').pop() === 'jpeg'
                            || file.split('.').pop() === 'jpg'
                            || file.split('.').pop() === 'gif' ">
